# Summary

[Introduction](README.md)  

- [Libuptest introduction](my-first-chapter.md)    
    - [Libuptest connect to a chain with JsonRPCClient](code/jsonrpc.md)  
    - [Libuptest types](code/types.md)    
    - [Libuptest ws_mod helper](code/ws_mod.md)   
    - [Libuptest Decoding Extrinsics](code/decode.md)   
- [Uptest command line tool](cli/README.md)   
    - [Watch for runtime upgrades and custom events](cli/watch_runtime_upgrade.md)   
    - [Subscribe to block events](cli/event_sub.md)
- [Libuptest code examples](code_samples/README.md)   
    - [Check added and changed storage items after a runtime upgrade](code_samples/upgrade_change_diff.md)
    - [Using Libuptest with Subxt](code_samples/subxt.md)
    - [Libuptest random input generation](code_samples/random.md)
- [Libuptest in Action](action/readme.md)  
    - [Tutorial #1 preventing breaking upgrades](action/tutorial_1.md)   
    - [Submitting runtime upgrades](action/submit.md) 
- [Real chains breaking - Case studies](case_study/readme.md)   


